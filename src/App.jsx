...
function App() {
  ...
  // var showFilterData = "";
  var showFilterData = filterArray.map((old) => {
    return (
      <div className="box" key={old.id}>
        <div>Name: {old.name}</div>
        <div>Gender: {old.gender}</div>
  ...
  function filterResult(val) {
    val == 1 ? setFilterOn(false) : setFilterOn(true);
    // console.log(filterOn)
    var temp = dataArray.filter((old) => {
      return val == old.department;
    });
    console.log(temp);
    setFilterArray(temp);
  }
  ...
  return (
    <div className="App">
      ...
      <hr />
      <button onClick={() => sortedResult("Asc")}>Sort By Salary Ascending</button>
      <button onClick={() => sortedResult("Des")}>Sort By Salary Descending</button>
      <hr />
